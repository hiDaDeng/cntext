...
WEPA, the Word Embedding Projection Approach, is a theory-driven semantic projection workflow for measuring construct-related linguistic salience in user-generated text. It represents a construct as a semantic axis in a word-embedding space using positive pole and negative pole anchor words, then projects text onto that axis.

cntext supports the core WEPA workflow:

1. Train or load domain-appropriate embeddings with `ct.Word2Vec`, `ct.GloVe`, or `ct.load_w2v`.
2. Define theory-driven anchor words for the positive pole and negative pole.
3. Construct a semantic axis with `ct.generate_concept_axis`.
4. Score text with `ct.project_text` or the convenience wrapper `ct.wepa`.
...
score = ct.project_text(wv, "commit persist focus goal", axis, lang="english")
```
...
Additional WEPA documentation is available in `docs/wepa.md`. The paper-focused reproducibility guide is in `docs/paper_reproducibility.md`, and anchor dictionary format guidance is in `examples/wepa_anchor_dictionary_format.md`.
...
> Deng, D. (forthcoming). Measuring Psychological Constructs from Social Media Text Using the Word Embedding Projection Approach.
